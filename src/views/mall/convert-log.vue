...
<template>
  <div class="app-container">
    <el-tabs value="1">
      <el-tab-pane :disabled="true" label="兑换记录" name="1">
        <!--操作控件-->
        <el-card :body-style="{ padding: '10px 20px' }">
          <el-form :inline="true" :model="sortOptions" :rules="sortFormRules" ref="dataForm" class="sort-form-inline">
        ...
        <el-table v-loading="isLoading" :data="tableData" border style="width: 100%">
          ...
          <el-table-column align="center" label="昵称" width="">
            <template slot-scope="scope">
              <router-link
                :to="{ name: 'convert-user-check', params: { from: 'consume-list', userData: userCheckDataAdaptor(scope.row) } }"
              >
          ...
          <el-table-column align="center" label="订单号" width="">
            <template slot-scope="scope">
              ...
              <router-link
                ...
                :to="{ name: 'convert-order-check', params: { from: 'consume-list', orderData: scope.row } }"
              >
        ...
        onSort(){
            ...
            this.$refs['dataForm'].validate((valid) => {
                if (valid) {
                    ...
                    const logList = {curtPage:(this.currentPage), pageSize:this.pageSize, orderNo: this.sortOptions.orderNumber,userId:this.sortOptions.tableUserId,phone: this.sortOptions.telephone,
                    exchangeTime:(this.sortOptions.exchangeTime && this.sortOptions.exchangeTime!=null)?(this.convertToTimeString(this.sortOptions.exchangeTime).slice(0,10)):(""), recordType: 'subtract'};

                    getLogList(logList).then(response=> {
        ...
        getLastSortOptions(){
            ...
            if(lastSortOptions&&lastSortOptions.length>0)
            {
                ...
                for(const oneProp in this.sortOptions)
                {
                  ...
                  {
                    this.sortOptions[oneProp] = (lastSortOptionsObj.hasOwnProperty(oneProp))?lastSortOptionsObj[oneProp]:((this.sortOptions[oneProp] instanceof Array)?[]:"");
                  }
                }
            }
        },
