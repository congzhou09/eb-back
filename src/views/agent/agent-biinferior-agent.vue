...
import {getBiinferiorAgent} from '../../api/agent';
export default{
    props:{
        agentID:{
            type: Number
        ...
        onSort(){
            this.$refs['sortForm'].validate((valid) => {
                if (valid)
                {
                    ...
                    getBiinferiorAgent(queryInfo).then(response=> {
                        if (response.status === 200) {
                            if (response.data.result) {
                                ...
                                this.$message({
                                    type: 'error',
                                    message: `获取下下级代理失败：${response.data.message}`,
                                    duration: 1800
                                });
                            }
                        }
                            ...
                            this.$message({
                                type: 'error',
                                message: `获取下下级代理失败 ${response.status}：${response.statusText}`,
                                duration: 1800
                            });
                        }
                        ...
                        this.$message.error("获取下下级代理失败:" + err.message);
                        this.isLoading = false;
                    });
                }
                else
