<template>
  <div class="app-container">
    <el-tabs value="first">
      <el-tab-pane :disabled="true" label="banner位设置" name="first">
        <!--操作控件-->
        <el-card :body-style="{ padding: '10px 20px' }">
          <router-link :to="{ name: 'banner-add' }">
            <el-button class="add-btn">新增banner位</el-button>
          </router-link>
        </el-card>
        ...
        <el-table v-loading="isLoading" :data="tableData" border style="width: 100%">
          ...
          <el-table-column align="center" prop="name" label="banner名称" width=""> </el-table-column>
          <el-table-column align="center" label="操作">
            <template slot-scope="scope">
              <router-link :to="{ name: 'banner-edit', params: { bannerEditData: scope.row } }">
...
<script type="text/ecmascript-6">
    import {getBannerList, deleteBanner, changeBannerSort} from "../../api/activity";
    ...
    export default{
        ...
        methods:{
            ...
            onDelete(item){
                this.$confirm(`将删除banner位 “${item.name}” , 是否继续?`, '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    const bannerInfo = {id: item.id};
                    deleteBanner(bannerInfo).then(response=>{
                        if(response.status===200)
                        {
                            if(response.data.result)
                            {
                                this.$message({
                                    type: 'success',
                                    message: `banner位 “${item.name}” 删除成功!`,
                                    duration: 1800
                                });
                            ...
                            else
                            {
                                this.$message({
                                    type: 'success',
                                    message: `banner位 “${item.name}” 删除失败：${response.data.message}`,
                                    duration: 1800
                                });
                            }
                        }
            ...
            onUpMove(index, item){
                if(index > 0)
                {
                    ...
                    changeBannerSort(sortInfo).then(response =>{
                        if(response.status===200)
                        {
                            if(response.data.result)
                            {
