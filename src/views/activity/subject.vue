<template>
  <div class="app-container">
    <el-tabs value="first">
      <el-tab-pane :disabled="true" label="专题设置" name="first">
        <!--操作控件-->
        <el-card :body-style="{ padding: '10px 20px' }" v-if="!isReadOnly">
          <router-link :to="{ name: 'subject-add' }">
        ...
        <el-table v-loading="isLoading" :data="tableData" border style="width: 100%">
          ...
          <el-table-column align="center" prop="name" label="专题名称" width=""> </el-table-column>
          <el-table-column align="center" label="操作" v-if="!isReadOnly">
            <template slot-scope="scope">
              <router-link :to="{ name: 'subject-edit', params: { editData: scope.row } }">
...
<script type="text/ecmascript-6">
    import {getSubjectList, deleteSubject, changeSubjectSort} from '../../api/activity';
        ...
        methods:{
            onRefresh(){
                ...
                getSubjectList().then(response=> {
                    if (response.status === 200) {
                        ...
                        this.$message({
                            type: 'error',
                            message: `获取专题列表失败 ${response.status}：${response.statusText}`,
                            duration: 1800
                        });
                    }
            ...
            onDelete(item){
                this.$confirm(`将删除专题 “${item.name}” , 是否继续?`, '提示', {
                    ...
                    const subjectInfo = {id: item.id};
                    deleteSubject(subjectInfo).then(response=> {
                        if (response.status === 200) {
                            ...
                            this.$message({
                                type: 'error',
                                message: `专题删除失败 ${response.status}：${response.statusText}`,
                                duration: 1800
                            });
                        }
                        this.onRefresh();
            ...
            onUpMove(index, item){
                if(index > 0)
                {
                    ...
                    changeSubjectSort(sortInfo).then(response =>{
                        if(response.status===200)
                        {
                            if(response.data.result)
                            {
