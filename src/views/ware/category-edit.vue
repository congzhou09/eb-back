...
import { editCategory } from '../../api/ware';
export default {
  components: {
    categoryAddContent
  },
  ...
  methods: {
    onSubmit(formData) {
      const categoryInfo = { id: this.preData.id, name: formData.name, imageUrl: formData.coverImgUrl };
  ...
  created() {
    if (this.$route.params.categoryData && this.$route.params.categoryData.name.length > 0) {
      this.preData.id = this.$route.params.categoryData.id;
