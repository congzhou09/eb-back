...
export default {
  ...
  methods: {
    onTabClick(tab, event) {
      window.localStorage.setItem('wareListCurrentType', tab.name);
      window.localStorage.setItem('curPage', 1);
      window.localStorage.setItem('sortOptions', '');
      tab.$children[0].onRefresh();
    },
    curTabType() {
      const curListType = window.localStorage.getItem('wareListCurrentType');
  ...
  created() {
    if (this.$route.params.fromSiderBar) {
      window.localStorage.setItem('wareListCurrentType', 'all');
      window.localStorage.setItem('curPage', 1);
      window.localStorage.setItem('sortOptions', '');
    }
  }
